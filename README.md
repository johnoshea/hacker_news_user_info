...
- **Account age and karma** pulled from HN's public API, e.g. `(7 years old, 12345 karma)`. Accounts younger than 6 months show this blurb in green so brand-new users stand out (the threshold is the `NEW_ACCOUNT_MAX_AGE_MS` constant in `src/config.js`).
...
- **Highlighted unread comments**: comments that weren't on the page the last time you visited the same thread get a faint orange tint in their indent gutter. The "seen" list is per-item with a 3-day TTL. Only a tab you actually look at counts as a visit — a story opened in a background tab and never switched to doesn't mark its comments as read (the same rule applies to both watch features below).
- **`collapse seen`**: on a thread you've read before, a `collapse seen (N new)` link appears in the item's subtext. Click it and every comment you've already read shrinks to its header line, while any branch containing nothing new disappears behind a `[N hidden]` link. What's left at full size is the new comments and the chain of parents leading down to them, so a reply still reads in context. Click `[N hidden]` to bring a branch back, or click a collapsed comment's indent gutter to read just that one. Click `show all` to restore the page. It starts off on every load, and it never touches HN's own collapse state.
